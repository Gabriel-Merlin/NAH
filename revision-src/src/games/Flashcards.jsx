  ...
  return (
    <div className="card p-5">
      ...
      <div className="[perspective:1200px]">
        ...
        >
          <span className="absolute inset-0 flex flex-col items-center justify-center rounded-2xl border-2 border-slate-200 bg-white p-6 text-center [backface-visibility:hidden] dark:border-slate-700 dark:bg-slate-800">
            <span className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-400">{t('question')}</span>
            <span className="text-xl font-bold">{card.front}</span>
            <span className="mt-4 text-xs text-slate-400">{t('tapToFlip')}</span>
          </span>
          <span
          ...
          >
            <span className="mb-2 text-xs font-bold uppercase tracking-wide" style={{ color }}>{t('answer')}</span>
      ...
      {flipped ? (
        ...
        <button onClick={() => setFlipped(true)} className="btn-ghost mt-4 w-full">{t('seeAnswer')}</button>
      )}
    </div>
  )
}
